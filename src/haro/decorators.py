...
def call_when_sls_haro_not_added(func):
    """
    Serverless を使った新 haro がチャンネルに追加されていないときのみメソッドを実行するデコレータ

    新 haro に移植済なコマンドのメソッドに付けて、旧 haro と新 haro が一緒に追加されているチャンネルでは、
    新 haro 側のコマンドが優先されるように制御します

    NOTE: デコレータは下から順に呼ばれるので、このデコレータは respond_to, listen_to よりも前に配置すること

    @call_when_sls_haro_not_added
    @respond_to("〜")
    def method(message):
        pass
    """
